...
BEGIN
    ...
    IF EXISTS(select 1 FROM Stage.plan_temp pt)
        BEGIN
            ...
            INSERT INTO dar.[plan] (Версія, Год, Месяц, Матеріал, QTY, Валюта, SUM_CURR, SUM_UAH, SUM_CURR_AE,
                                    SUM_UAH_AE)
            SELECT TRY_CAST(pt.VER_PLAN AS NVARCHAR(10))             VER_PLAN
                 ...
                 , TRY_CAST(replace(pt.PRICE_CURR_AE, ',', '.') AS FLOAT) *
                   TRY_CAST(replace(pt.QTY, ',', '.') AS float)      SUM_CURR_AE
                 , TRY_CAST(replace(pt.PRICE_UAH_AE, ',', '.') AS FLOAT) *
                   TRY_CAST(replace(pt.QTY, ',', '.') AS float)      SUM_UAH_AE
