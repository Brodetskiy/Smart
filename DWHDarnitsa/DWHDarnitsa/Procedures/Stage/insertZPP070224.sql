create PROCEDURE [stage].[insertZPP070224]
	AS
		begin
			...
			insert into Production.ZPP070224 ([наименован], [код ресурса], [номер заказа], материал, [наименование материала], серия, [текст операции], [дата завершення], количество, количество1, количество2, еи, [общее время работы ресурса], [время на переналадки], [время перехода на след серию], [время на производство], [плановая производительность], [коеф пересчета], [общая эффективность], [эффективность без учета], производительность, качество, доступность, [доступность д1], [доступность д2], [доступность д3])
select --top 1000
       [Наименован],
       [код ресурса],
       ...
       [серия],
       [текст операции],
       cast(substring([Дата завершення],7, 4) + substring([Дата завершення],4, 2) + substring([Дата завершення],1, 2) as date) [Дата завершення],
       ...
       iif(charindex('-', [доступность д3]) = 0, cast(replace(replace([доступность д3], ',', '.'), ' ', '') as float), cast(replace(replace(replace([доступность д3], ',', '.'), ' ', ''), '-', '') as float) * -1)
